var global_circleMoving = false;

function addBayesCircle(bayesVar)
{
    if (notDef(bayesVar.varCircles)) {
        bayesVar.varCircles = [];
        bayesVar.activeCircles = [];
    }
    var newVarCircle = makeBayesCircle(20, bayesVar.color);
    newVarCircle.bayesVar = bayesVar;
    ...
    bayesVar.varCircles.push(newVarCircle)
    return newVarCircle;
}

function removeBayesCircle(varCircle)
{
    var bayesVar = varCircle.bayesVar;
    bayesVar.varCircles.deleteFirstElem(varCircle);
}

function getFromVars(bayesVar)
{
    var fromVars = Set();
    for (var i=0; i<bayesVar.varCircles.length;i++)
    {
        for (var j=0;j<bayesVar.varCircles[i].fromNodes.length;j++)
        {
            var fromVar = bayesVar.varCircles[i].fromNodes[j][0].bayesVar;
...
function connectCircles2(fromCircle, toCircle)
{
  ...
  //  } else {
    ...
    fromVars = getFromVars(arrow.toNode.bayesVar);
    //alert(fromVars)
    fromNames = [];
    for (var i=0; i<fromVars.length; i++) {
        fromNames.push(fromVars[i].varText)
    ...
    arrow.toNode.bayesVar.probSetter = makeSwitcher(arrow.toNode.bayesVar, BAYES_HIST_WIDTH, BAYES_HIST_HEIGHT,
            arrow.toNode.bayesVar.color, fromNames, arrow.toNode.bayesVar.getCurrHistVals());
...
function disconnectCircles(fromCircle, toCircle)
{
    ...
    var fromVars = getFromVars(toCircle.bayesVar);
