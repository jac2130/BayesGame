...
function makePutButton(betView)
{
    ...
    var callback = {
        "put": betView.bettingMenu,
        "points": betView.pointMenu,
        'betView': betView,
        'truth': truth,
        "call": function() {
            var thePut = this.put.activeChoice;
            var points = this.points.activeChoice;
            var betView = this.betView;
            var bayesVar = betView.bayesVar;
            //alert(truth[truth.length-1].period);
            if (bayesVar.possibilities.indexOf(thePut) >= 0 && points in range(91))
            { 
                $.ajax({
                    type: "post",
                    url: '/ajax/newput',
...
function makeContractedButton(inpWindow)
{
    ...
    var callback = {
        ...
        "call": function() {
            ...
            frameWindow.render(topLayer.shape, {x:frameWindow.xPos, y:stageHeight-frameWindow.height-contractedButton.height});
            this.button.erase();
        }
    };
    ...
    renewData = function()
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                var totalWinnings=0;
                //alert("prev prize_door: " + truth[truth.length-2]['prize_door']);
                //alert("prev share_door: " + share_door);
                //alert("prev monty_door: " + truth[truth.length-2]['monty_door']);
                //alert("new monty_door: " + truth[truth.length-1]['monty_door']);
                if (truth[truth.length-2]['prize_door']===share_door)
                {
                    //alert("you just won "+ p.price+ " points")
                ...
                {
                    //alert("you just lost "+ p.price+ " points")
                    //pointWindow.points.changeText(user.points +"      points");
                    totalWinnings = 0;
                    //alert("you lost");
                }
...
{ 
    ...
    //this is the call window, as distinct from the puts window!
    var betWidth = 100;
    var betHeight=40
    var betSpacing = 10;
	...
	view.bets.map(function(bet)
    {
        ...
        if (postItem.bet.userid !== user.id)
        {
            ...
            acceptButton.text = makeTextWidget("ACCEPT", 10, "Arial", "white");
            ...
            var acceptButtonCallBack = {
                ...
                "postItem": postItem,
                "call": function() {
                   // alert("postItem userId: " + this.postItem.bet.userid + "\n" +     "postItem id: " + this.postItem.bet.id);
                    $.ajax({
                        type: "post",
                        url: '/ajax/accept_call/' + this.postItem.bet.id + '/' + user.id + '/' + this.postItem.bet.userid,
                        async: true,
        ...
        for (var i=0;i<frame.postItems.length;i++)
        {
            ...
            frame.postItems[i].renderW(frame, {x:10, y:view.height-frame.postItems[i].yCoord});
    ...
    var count=0;
    previousTimeT1 = getTime();
    function callTick(event)
    { 
        var currentTimeT1 = getTime();
        if (currentTimeT1-previousTimeT1 > 500) {
            previousTimeT1 = currentTimeT1;
            $.ajax({
                type: "GET",
                ...
                success: function(data) {
                    //do your stuff with the JSON data;
                    //alert(user_data.myposts.length !== data.myposts.length);
                    if (user_data.myposts.length !== data.myposts.length) {
                        alert("NUMBER OF CALLS CHANGED");
                        alert("user_data.myposts.length: " + user_data.myposts.length);
                        alert("data.myposts.length: " + data.myposts.length);

                        //view.refresh(user_data.myposts);
                        //alert(posts)
                    }
                }
            });
    ...
    createjs.Ticker.on("tick", callTick);
