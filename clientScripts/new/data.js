...
function makePrediction(variable, t, width)
{
	...
	$.ajax({
	    type: "GET",
	    url: '/js/query/'+ user.id + queryPath,
	    async: true,
...
betQueryWindow.dict[key].key=makeTextWidget(key, 10, "Arial", "#666");

betQueryWindow.dict[key].render(betQueryWindow.shape, {x:xPos, y:betQueryWindow.height-100-(betQueryWindow.height-120)*currentData[key]})
				
betQueryWindow.dict[key].percent.render(betQueryWindow.shape, {x:xPos+barWidth/2-betQueryWindow.dict[key].percent.width/2, y:betQueryWindow.height-110-(betQueryWindow.height-120)*currentData[key]})

betQueryWindow.dict[key].key.render(betQueryWindow.shape, {x:xPos+barWidth/2-betQueryWindow.dict[key].key.width/2, y:betQueryWindow.height-95})
		    ...
		    success: function(){
                ...
                if (obtainedTruth) {
                    $.ajax(
                        {
                        type: "GET", 
                        url:'/js/hasNewData/' + modelClass["model_name"] + "/" + dataWindow.data[dataWindow.data.length-1].period, 
                        async: true,
